...
async def run_admin_e2e_test():
    print("===========================================")
    print("  Apartment Ultra 运营端 E2E 测试")
    print("===========================================\n")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if await add_btn.is_visible(timeout=3000):
                ...
                name_input = page.locator('input[placeholder*="名称"], input[placeholder*="组织"], input[placeholder*="name"]').first
                if await name_input.is_visible(timeout=3000):
                    await name_input.fill(f"测试组织{random.randint(100000, 999999)}")
            ...
            if await add_btn.is_visible(timeout=3000):
                ...
                address_input = page.locator('input[placeholder*="地址"], input[placeholder*="address"]').first
                if await address_input.is_visible(timeout=3000):
                    await address_input.fill("北京市朝阳区测试路100号")
            ...
            room_row = page.locator('text=201').first
            ...
            if await add_btn.is_visible(timeout=3000):
                ...
                if await phone_input.is_visible(timeout=3000):
                    await phone_input.fill(f"137{random.randint(10000000, 99999999)}")
                
                id_input = page.locator('input[placeholder*="身份证"], input[placeholder*="ID"]').first
                if await id_input.is_visible(timeout=3000):
                    await id_input.fill("110101199001011234")
            ...
            if await add_btn.is_visible(timeout=3000):
                ...
                if await room_select.is_visible(timeout=3000):
                    ...
                    await sleep(500)
                    room_option = page.locator('div[role="option"]:has-text("201"), li:has-text("201")').first
                    if await room_option.is_visible(timeout=3000):
                        await room_option.click()
                        await sleep(500)
                
                # 选择租客
                tenant_select = page.locator('input[placeholder*="租客"], input[placeholder*="tenant"], [role="combobox"]').last
                if await tenant_select.is_visible(timeout=3000):
                    await tenant_select.click()
                    await sleep(500)
                    tenant_option = page.locator('div[role="option"]:has-text("李四"), li:has-text("李四")').first
                    if await tenant_option.is_visible(timeout=3000):
                        await tenant_option.click()
                        await sleep(500)
                ...
                # 填写押金
                deposit_input = page.locator('input[placeholder*="押金"], input[placeholder*="deposit"]').first
                if await deposit_input.is_visible(timeout=3000):
                    await deposit_input.fill("6000")
            ...
            if await bill_rows.first.is_visible(timeout=3000):
                ...
                pay_btn = page.locator('button:has-text("收款"), button:has-text("支付"), button:has-text("付款"), button:has-text("核销")').first
            ...
            print(f"\n❌ 测试失败: {e}")
            await page.screenshot(path="/root/.openclaw/workspace/apartment-ultra/admin-error-screenshot.png")
            print("错误截图已保存到 admin-error-screenshot.png")
            raise
        finally:
            await browser.close()

if __name__ == "__main__":
    asyncio.run(run_admin_e2e_test())
