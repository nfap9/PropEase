...
本文档为 E2E 详细规范，与 [e2e/AGENTS.md](../e2e/AGENTS.md) 配合使用；日常编写以 AGENTS.md 为主，此处为补充与详细示例。约定 Playwright E2E 测试的写法，以保证用例可运行、可维护并与 [测试用例.md](./测试用例.md) 对齐。
...
- **运行**：先启动 API（端口 8000）与前端（端口 3000），例如 `pnpm run dev`；业务端密码登录需以 `SEED_E2E_USER=true` 启动 API 一次以创建测试用户。执行 `pnpm run test:e2e` 或 `pnpm exec playwright test business`（仅业务端）。
...
- 建议在 `test.describe` 或 test 名称中标注对应 [测试用例.md](./测试用例.md) 的编号或模块，便于追溯，例如「业务端 - 公寓管理（对应测试用例 3.1、3.2）」、test 名末尾「（APT-C-01）」。
