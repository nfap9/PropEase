...
export { useMonthStats } from './use-month-stats';
...
export function useUtilitiesData() {
  ...
  const createUtility = useCallback(
    (data: Parameters<typeof utilitiesApi.create>[0], onSuccess?: () => void) => {
  ...
  const updateUtility = useCallback(
    (id: string, data: Parameters<typeof utilitiesApi.update>[1], onSuccess?: () => void) => {
      updateMutation.mutate({ id, data }, { onSuccess });
    },
    [updateMutation],
  );

  const batchImportUtilities = useCallback(
    (data: Parameters<typeof utilitiesApi.batchCreate>[0], onSuccess?: () => void) => {
      batchImportMutation.mutate(data, { onSuccess });
    },
    [batchImportMutation],
  );
