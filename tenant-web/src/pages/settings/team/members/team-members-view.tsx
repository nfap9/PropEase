...
import { Button, Modal, Input, Skeleton, Tag, Select, Dropdown, Form, type MenuProps } from 'antd';
...
export function TeamMembersView() {
  ...
  const memberColumns: ColumnsType<OrganizationMember> = [
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-muted">
            {record.user_full_name?.charAt(0).toUpperCase() || 'U'}
          </div>
          <div>
            <div className="font-medium">{record.user_full_name || tenantMessages.common.unknownUser}</div>
            <div className="text-sm text-muted-foreground">{record.user_phone || '-'}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      minWidth: 60,
      render: (_, record) => {
        if (record.user_id === user?.id || !canManage) return null;
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            <Button onClick={() => setIsInviteOpen(false)}>取消</Button>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button onClick={() => setIsRemoveMemberOpen(false)}>取消</Button>
            <Button
              danger
              type="primary"
