/**
 * ApartmentDetailView - 公寓详情视图
 *
 * 自包含视图，内部管理：
 * - useApartmentDetailData：所有 mutations
 * - 7 个弹窗状态
 * - 批量选择房间状态
 * - Tabs 切换
 */
...
import { useApartmentDetailData, useRoomBatchSelection, useApartmentRoomMetrics } from '@/pages/apartments/hooks/use-apartment-detail';
...
export function ApartmentDetailView() {
  ...
  const handleToggleAll = useCallback((select: boolean) => {
    if (!select) setBatchSelectedRooms(new Set());
  }, []);
