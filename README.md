...
当 `NODE_ENV !== 'production'`（本地 `pnpm run dev` 默认即为开发环境）时，以下功能会自动启用模拟/跳过，方便完成完整流程：

| 功能       | 开发环境行为                                               |
|------------|------------------------------------------------------------|
| **验证码** | 使用固定码 `123456`，无需真实短信；点击「获取验证码」后控制台会提示 |
| **订阅支付** | 无微信支付配置时，支付页显示「模拟支付」按钮，一键完成订单并开通订阅 |

生产环境需配置微信支付与短信服务；`api/.env` 中可选 `IS_DEV=true` 强制启用开发模式。
