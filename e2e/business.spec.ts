 ...
 * 已标注 TODO 的地方表示前端添加对应 testid 后可以改用 testid
 */

test.describe('业务端 - 侧栏导航', () => {
  test('从仪表盘可进入公寓管理', async ({ page }) => {
    await page.goto('/dashboard');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NAV.APARTMENTS)
    await page.getByRole('link', { name: '公寓管理', exact: true }).click();
    await expect(page).toHaveURL(/\/apartments$/);
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(APARTMENTS.HEADING)
    await expect(page.getByRole('heading', { name: '公寓管理' })).toBeVisible();
  });

  test('从仪表盘可进入全部房间', async ({ page }) => {
    await page.goto('/dashboard');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NAV.ROOMS)
    await page.getByRole('link', { name: '全部房间' }).click();
    await expect(page).toHaveURL(/\/rooms$/);
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.HEADING)
    await expect(page.getByRole('heading', { name: '全部房间' })).toBeVisible();
  });

  test('从仪表盘可进入租客管理', async ({ page }) => {
    await page.goto('/dashboard');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NAV.TENANTS)
    await page.getByRole('link', { name: '租客管理' }).click();
    await expect(page).toHaveURL(/\/tenants$/);
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TENANTS.HEADING)
  ...
  test('从仪表盘可进入通知', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NAV.NOTIFICATIONS)
    await page.locator('header').getByRole('link', { name: '通知' }).click();
    await expect(page).toHaveURL(/\/notifications$/);
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NOTIFICATIONS.HEADING)
...
test.describe('业务端 - 通知（对应测试用例 11）', () => {
  test('通知页有标题和列表或空状态（NT-L-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为
    // const hasList = await page.getByTestId(NOTIFICATIONS.LIST).count() > 0;
    // const hasEmpty = await page.getByTestId(NOTIFICATIONS.EMPTY_STATE).count() > 0;
  ...
  test('通知页有通知列表或暂无通知或全部标已读按钮（NT-C-01 相关）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NOTIFICATIONS.MARK_ALL_READ_BUTTON)
  ...
  test('单条未读通知可点击标为已读（NT-M-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NOTIFICATIONS.MARK_READ_BUTTON)
...
test.describe('业务端 - 公寓管理（对应测试用例 3.1、3.2）', () => {
  test('公寓管理页有标题且为列表或空状态（APT-L-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为
    // const hasList = await page.getByTestId(APARTMENTS.LIST).count() > 0;
    // const hasEmpty = await page.getByTestId(APARTMENTS.EMPTY_STATE).count() > 0;
  ...
  test('可打开新增公寓弹窗并看到表单', async ({ page }) => {
    await page.goto('/apartments');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(APARTMENTS.NEW_BUTTON)
    const newBtn = page.getByRole('button', { name: '新增公寓' }).first();
    if (await newBtn.isVisible()) {
      await newBtn.click();
      const dialog = page.getByRole('dialog');
      await expect(dialog.getByText('新增公寓')).toBeVisible();
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(APARTMENTS.NAME_INPUT)
      await expect(dialog.getByLabel('公寓名称')).toBeVisible();
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(APARTMENTS.ADDRESS_INPUT)
      await expect(dialog.getByLabel('地址')).toBeVisible();
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(COMMON.CANCEL_BUTTON)
  ...
  test('创建时公寓名称为必填（APT-C-02）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(APARTMENTS.CONFIRM_BUTTON)
  ...
  test('可编辑公寓并保存（APT-E-01）', async ({ page }) => {
    ...
    const newName = createUniqueName('E2E公寓_编辑');
  ...
  test('可删除公寓（APT-D-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByRole('button', { name: /^删除/ })
...
test.describe('业务端 - 全部房间', () => {
  test('全部房间页有标题和搜索框', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.HEADING)
    await expect(page.getByRole('heading', { name: '全部房间' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.SEARCH_INPUT)
...
test.describe('业务端 - 租客管理（对应测试用例 4）', () => {
  test('租客管理页有标题和新增租客按钮或列表', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TENANTS.HEADING)
    await expect(page.getByRole('heading', { name: '租客管理' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TENANTS.NEW_BUTTON)
  ...
  test('可打开新增租客弹窗', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TENANTS.NAME_INPUT)
  ...
  test('可创建新租客并出现在列表（TN-C-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TENANTS.PHONE_INPUT)
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TENANTS.CONFIRM_BUTTON)
    await dialog.getByRole('button', { name: '创建' }).click();
...
test.describe('业务端 - 租约管理（对应测试用例 5）', () => {
  test('租约管理页有标题和新增租约按钮或列表', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(LEASES.HEADING)
    await expect(page.getByRole('heading', { name: '租约管理' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(LEASES.NEW_BUTTON)
...
test.describe('业务端 - 水电录入（对应测试用例 6）', () => {
  test('水电记录页有标题和录入读数或批量导入按钮', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(UTILITIES.HEADING)
    await expect(page.getByRole('heading', { name: '水电记录' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(UTILITIES.ENTRY_BUTTON)
    const hasEntry = await page.getByRole('button', { name: '录入读数' }).count() > 0;
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(UTILITIES.BATCH_BUTTON)
...
test.describe('业务端 - 账单管理（对应测试用例 7）', () => {
  ...
  test('有待支付账单时可打开登记付款弹窗（BL-PY-C-01 相关）', async ({ page }) => {
    await page.goto('/bills');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(BILLS.PAY_BUTTON)
...
test.describe('业务端 - 组织/团队设置（对应测试用例 2.x）', () => {
  test('团队设置可打开创建组织弹窗（ORG-C-01 相关）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.HEADING)
    await expect(page.getByRole('heading', { name: '团队设置' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.CREATE_ORG_BUTTON)
    ...
    if (await createBtn.isVisible()) {
      ...
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.ORG_NAME_INPUT)
  ...
  test('成员管理 Tab 下可见成员列表或邀请成员按钮（ORG-MB-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.HEADING)
    await expect(page.getByRole('heading', { name: '团队设置' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.INVITE_MEMBER_BUTTON)
    const hasInvite = await page.getByRole('button', { name: '邀请成员' }).count() > 0;
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.MEMBER_LIST)
    const hasRoleCol = await page.getByRole('columnheader', { name: '角色' }).count() > 0;
    const hasMemberText = await page.getByText('成员').count() > 0;
    const hasOrgList = await page.getByText('组织').count() > 0;
    // 接受多种可能的页面状态：有邀请按钮、有角色列、有成员文本、有组织列表
    expect(hasInvite || hasRoleCol || hasMemberText || hasOrgList).toBe(true);
  });
  ...
  test('可打开邀请成员弹窗（ORG-MB-02 相关）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.HEADING)
    await expect(page.getByRole('heading', { name: '团队设置' })).toBeVisible();
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(TEAM_SETTINGS.MEMBERS_TAB)
...
test.describe('业务端 - 设置', () => {
  test('设置首页有标题和团队设置、订阅管理等入口', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(SETTINGS.HEADING)
  ...
  test('可进入团队设置页', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(SETTINGS.TEAM)
  ...
  test('可进入权限管理页', async ({ page }) => {
    await page.goto('/settings');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(SETTINGS.PERMISSIONS)
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(PERMISSIONS.HEADING)
  ...
  test('可进入订阅管理页', async ({ page }) => {
    await page.goto('/settings');
    // 同屏存在多个相同 name 时使用 .first()
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(SETTINGS.SUBSCRIPTION)
...
test.describe('业务端 - 公寓详情（对应测试用例 3.2、3.4、3.6）', () => {
  ...
  test('公寓详情页可打开新增房间弹窗（RM-C-01 相关）', async ({ page }) => {
    ...
    await apartmentCard.click();
    await expect(page).toHaveURL(/\/apartments\/[^/]+/, { timeout: 10000 });
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.NEW_BUTTON)
  ...
  test('公寓详情页可打开批量添加房间弹窗（RM-C-02 相关）', async ({ page }) => {
    ...
    await apartmentCard.click();
    await expect(page).toHaveURL(/\/apartments\/[^/]+/, { timeout: 10000 });
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.BATCH_BUTTON)
  ...
  test('公寓详情页可打开费用配置弹窗（UC 相关）', async ({ page }) => {
    ...
    await apartmentCard.click();
    await expect(page).toHaveURL(/\/apartments\/[^/]+/, { timeout: 10000 });
    // TODO: 前端添加 data-testid 后添加费配置相关 testid
...
test.describe('业务端 - 完整业务流程', () => {
  test('主流程可完整走通并验证关键状态变化（BIZ-FLOW-01）', async ({ page }) => {
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.NUMBER_INPUT)
    await roomDialog.getByLabel(/房间号/).fill(roomNumber);
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.MONTHLY_RENT_INPUT)
    ...
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(ROOMS.CONFIRM_BUTTON)
    ...
    await page.waitForLoadState('networkidle');
    // TODO: 前端添加 data-testid 后改为 page.getByTestId(NAV.UTILITIES)
    const utilitiesLink = page.getByRole('link', { name: '水电录入' }).first();
    await expect(utilitiesLink).toBeVisible({ timeout: 10000 });
    ...
    if (await roomOption.isVisible().catch(() => false)) {
      ...
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(UTILITIES.WATER_READING_INPUT)
      await utilityDialog.getByLabel(/水表读数/).fill(String(waterReading));
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(UTILITIES.ELECTRICITY_READING_INPUT)
      await utilityDialog.getByLabel(/电表读数/).fill(String(electricityReading));
      // TODO: 前端添加 data-testid 后改为 page.getByTestId(UTILITIES.SAVE_BUTTON)
