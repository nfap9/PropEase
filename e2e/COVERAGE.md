...
| ORG-MIG-01 | 数据迁移 | P2 | ✅ 已实现 | organization.spec.ts |
...
| ADM-ST-03 | 品牌配置 | P2 | ✅ 已实现 | admin/users.spec.ts |
| ADM-ST-04 | 额度定价配置 | P2 | ✅ 已实现 | admin/users.spec.ts |
...
| 用例编号 | 用例名称 | 优先级 | 自动化状态 | 测试文件 |
|---------|---------|--------|------------|----------|
| CONC-01 | 并发创建同一房间 | P2 | ✅ 已实现 | integration/concurrency.spec.ts |
| CONC-02 | 并发修改账单 | P2 | ✅ 已实现 | integration/concurrency.spec.ts |
...
| e2e/business/organization.spec.ts | ORG | 16 |
